...
export class DecisionWorkflow {
  static shouldFinalize(candidate, evaluation, reason = 'message') {
    ...
    const hasConsensus = Boolean(evaluation.decision && evaluation.decision.length > 5);
    const noContradiction = !evaluation.contradictionDetected;
  ...
  static async observeMessage({ roomId, message, io }) {
    ...
    if (!activeCandidate) {
      if (state.isAnalyzing) return; // Prevent concurrent duplicate creation
    ...
    if (state.isReviewing) return; // Prevent concurrent reviews
